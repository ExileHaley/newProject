/**
 *Submitted for verification at BscScan.com on 2025-05-18
*/
...
contract ReceiveHelper {
    address public constant USDT = 0x55d398326f99059fF775485246999027B3197955;//0x55d398326f99059fF775485246999027B3197955
    constructor() {
...
contract TokenV2 is ERC20, Ownable{
    ...
    uint256 public GASMAX=500000;

    uint256 public MIN_PROCESS_AWARD = 1300e18;
    uint256 public MIN_LIQUIDITY = 50e18;
    uint256 public MIN_ADD_LIQUIDITY = 100e18;

    bool public canDes = true;
    uint256 public lastDes;
    uint256 public timeDes=1 hours;
    uint256 public desRate=40;
    uint256 public deslimit=10;
    ...
    constructor(
        ...
        pancakeRouter = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);//0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function setcanDes() external onlyOwner {
        canDes = !canDes;
    }
    ...
    function setdesConfig(uint256 _num,uint256 _num2, uint256 _num3, uint256 _num4) external onlyOwner {
        lastDes = _num;
        timeDes = _num2;
        desRate = _num3;
        deslimit = _num4;
    ...
    function setLimitConfig(uint256 _minLiquidity,uint256 _minProcessAward, uint256 _minAddLiquidity, uint256 _maxBuyUsdt, uint256 _maxgas) external onlyOwner {
    ...
    function _transfer(address from, address to, uint256 value) internal override{
        ...
        // === check add liquidity ===
        bool isAddLdx = (to == pancakePair && msg.sender == address(pancakeRouter));
        if(isAddLdx){
        ...
        updateHolder(from);
        updateHolder(to);
        try this.process(isExchange, GASMAX) {} catch {}
    }




    event desPair(uint256 amount,uint256 num,uint256 now,uint256 lasttime,uint256 tt);
    function des() public {
        if(canDes){
            uint256 balance = balanceOf(pancakePair);
            if(lastDes>0 && lastDes+timeDes < block.timestamp && balance>0){
                uint256 during = block.timestamp-lastDes;
                uint256 num = during/timeDes;
                if(num>0){
                    if(num>deslimit){
                        num=deslimit;
                    }
                    uint256 desAmount = balance*(desRate*num)/100000;
                    super._transfer(pancakePair, DEAD, desAmount);
                    IUniswapV2Pair(pancakePair).sync();
                    emit desPair(desAmount,num,block.timestamp,lastDes,7);
                    lastDes=lastDes+num*timeDes;
                }
            }
        }
    }
    ...
    event Process(address amount,uint256 num,uint256 now,uint256 lasttime,uint256 tt);
    function process(bool isExchange, uint256 gas) external {
        emit Process(msg.sender,gas,gas,gas,8);
        require(msg.sender == address(this), "FORBIDDEN"); // only this contract can call
         emit Process(msg.sender,gas,gas,gas,9);
        if (isExchange) return;
        uint256 totalLP = IERC20(pancakePair).totalSupply();
        uint256 dividendFee = balanceOf(address(this)) - txFee;
        emit Process(msg.sender,totalLP,dividendFee,MIN_PROCESS_AWARD,10);
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            emit Process(msg.sender,totalLP,dividendFee,lpBalance,11);
            if (lpBalance > 0) {
                uint256 amount = (dividendFee * lpBalance) / totalLP;

                emit Process(msg.sender,amount,dividendFee,lpBalance,12);
